...
Public Class frmProcessInfo
    ...
    Private Sub frmProcessInfo_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SetToolTip(Me.cmdActivateHeapEnumeration, "The enumeration of heaps can fail and cause a crash of LitePM. This will be fixed soon. You must then manually enable this feature for now.")
            ...
            If _frmMain.saveDial.ShowDialog = Windows.Forms.DialogResult.OK Then
                ...
                If Len(s) > 0 Then
                    Using stream As New System.IO.StreamWriter(s, False)
                        ...
                        stream.Close()
                    End Using
                    Misc.ShowMsg("Save log", "Saved file successfully.", MessageBoxButtons.OK)
                End If
            End If
        Catch ex As Exception
        ...
        If IO.Directory.Exists(sDir) Then
            For Each it As cMemRegion In Me.lvProcMem.GetSelectedItems
                ...
                If IO.File.Exists(sFile) Then
                    okToReplace = (Misc.ShowMsg("Dump memory", "Would you like to replace the existing file ?" & " File " & sFile & " already exists.", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes)
                End If
                If okToReplace Then
                    it.DumpToFile(sFile)
                End If
...
#Region "Find panel"
    ...
    Private Sub cmdHideFindPanel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdHideFindPanel.Click
        ...
        If My.Settings.FirstTimeShowFindWindowWasClosed Then
            ...
            Misc.ShowMsg("Search panel", "You have closed the search panel." & " Press Ctrl+F on a listview to open it again.", MessageBoxButtons.OK)
            My.Settings.Save()
        End If
        Call hideFindPanel()
    End Sub
